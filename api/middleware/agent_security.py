...
class AgentSecurity:
    ...
    def require_roles(required_roles: List[str]):
        ...
        async def verify_agent_action(
            request: Request, user: dict = Depends(get_okta_user)
        ):
            ...
            logger.info(
                f"[AGENT SECURITY] Visibility: Entity '{agent_id}' from '{client_host}' is accessing '{request.url.path}'."
            )
            ...
            if not has_required_role:
                logger.warning(
                    f"[AGENT SECURITY] Connection Control Blocked: Entity '{agent_id}' lacks required roles {required_roles}."
                )
                raise HTTPException(
            ...
            logger.info(
                f"[AGENT SECURITY] Action Approved for '{agent_id}' on '{request.url.path}'."
            )
            return user
