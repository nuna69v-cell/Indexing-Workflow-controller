...
class GitHubAppManager:
    ...
    """
    def __init__(self, app_id: Optional[str] = None, private_key_path: Optional[str] = None, pat_token: Optional[str] = None):
        self.app_id = app_id or os.getenv("GITHUB_APP_ID")
        self.private_key_path = private_key_path or os.getenv("GITHUB_APP_PRIVATE_KEY_PATH")
        ...
        if not self.pat_token and not (self.app_id and self.private_key_path):
            raise ValueError("Must provide either a PAT token or both App ID and Private Key Path.")
    ...
    def list_repositories(self, installation_id: Optional[str] = None) -> List[Dict]:
        ...
        data = response.json()
        return data.get('repositories', data) if isinstance(data, dict) and 'repositories' in data else data

    def create_environment(self, owner: str, repo: str, environment_name: str, installation_id: Optional[str] = None) -> Dict:
    ...
    def get_repo_public_key(self, owner: str, repo: str, installation_id: Optional[str] = None) -> Dict:
    ...
    def create_repository_secret(self, owner: str, repo: str, secret_name: str, encrypted_value: str, key_id: str, installation_id: Optional[str] = None) -> bool:
    ...
    def create_environment_secret(self, owner: str, repo: str, env_name: str, secret_name: str, encrypted_value: str, key_id: str, repository_id: int, installation_id: Optional[str] = None) -> bool:
