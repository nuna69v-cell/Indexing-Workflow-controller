...
class OneDriveUploader:
    def __init__(self, client_id=None, tenant_id=None):
        ...
        if not self.client_id:
            logger.warning("ONEDRIVE_CLIENT_ID is not set. Authentication will likely fail.")
    ...
    def authenticate(self):
        ...
        if not result:
            logger.info("No suitable token found in cache. Starting Device Code Flow...")
            flow = self.app.initiate_device_flow(scopes=SCOPES)
            if "user_code" not in flow:
                raise ValueError(f"Failed to create device flow. Error: {json.dumps(flow, indent=4)}")
    ...
    def upload_file(self, local_path: str, remote_folder: str = None):
        ...
        remote_folder = remote_folder or self._get_config("ONEDRIVE_FOLDER_NAME") or "GenX_Signals"
        ...
        if response.status_code in [200, 201]:
            logger.info(f"Successfully uploaded {filename}. File ID: {response.json().get('id')}")
            return True
        else:
