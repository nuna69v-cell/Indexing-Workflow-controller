...
class TestScalpingService(unittest.TestCase):
    ...
    def test_15m_sell_signal(self, mock_rsi, mock_bbands):
        ...
        mock_bbands.return_value = (pd.Series(upper), pd.Series(middle), pd.Series(lower))
        ...
        # High >= Upper
        self.df['high'] = 106.0
        self.df['low'] = 100.0  # Ensure low is NOT <= lower band (95.0) to avoid entering Long block
        self.df['close'] = 104.0 # Doesn't matter for touch check which uses high/low depending on implementation
    ...
    def test_30m_buy_signal(self, mock_ema, mock_macd):
        ...
        mock_macd.return_value = (pd.Series(macd_line), pd.Series(signal_line), pd.Series(hist))
