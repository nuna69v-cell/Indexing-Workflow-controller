...
async def close_open_positions():
    print(
        f"[{datetime.now(tz_gmt3)}] ACTION: Closing all open positions on affected instruments (EURUSD, AUDUSD, GBPUSD, USDJPY, USDCAD, USDCHF, XAUUSD, XAGUSD)..."
    )
...
async def notify_admin_for_manual_update():
    print(
        f"[{datetime.now(tz_gmt3)}] ALERT: Administrator intervention required! Please log in to CapitalXtend Client Area -> Manage Account -> Change Leverage -> Select 'Unlimited'."
    )
...
async def schedule_leverage_update():
    ...
    print(
        f"Sleeping for {time_to_wait} seconds until {pre_update_time} to perform pre-update checks..."
    )
