...
for filepath in files:
    ...
    if "sudo apt-get update && sudo apt-get install -y sqlite3" in content:
        old_block = "sudo apt-get update && sudo apt-get install -y sqlite3"
        new_block = """sudo apt-get update
          sudo apt-get install -y build-essential wget sqlite3 libopenblas-dev
          wget https://prdownloads.sourceforge.net/ta-lib/ta-lib-0.4.0-src.tar.gz
          tar -xzf ta-lib-0.4.0-src.tar.gz
          cd ta-lib
          ./configure --prefix=/usr
    ...
    if 'copilot' in filepath:
        old_cmd = """        sudo apt-get update
        sudo apt-get install -y sqlite3"""
        new_cmd = """        sudo apt-get update
        sudo apt-get install -y build-essential wget sqlite3
        wget https://prdownloads.sourceforge.net/ta-lib/ta-lib-0.4.0-src.tar.gz
        tar -xzf ta-lib-0.4.0-src.tar.gz
        cd ta-lib
        ./configure --prefix=/usr
    ...
    if "export LD_LIBRARY_PATH=/usr/lib:$LD_LIBRARY_PATH" not in content and "python -m pytest tests/ -v" in content:
        content = content.replace("python -m pytest tests/ -v", "export LD_LIBRARY_PATH=/usr/lib:$LD_LIBRARY_PATH\n        python -m pytest tests/ -v")
    if "export LD_LIBRARY_PATH=/usr/lib:$LD_LIBRARY_PATH" not in content and "PYTHONPATH=. pytest --cov=src/ tests/" in content:
