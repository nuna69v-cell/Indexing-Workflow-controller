...
def mock_env_vars():
    with patch.dict(
        os.environ, {"STREAM_API_KEY": "mock_key", "STREAM_API_SECRET": "mock_secret"}
    ):
        yield
...
def test_stream_service_initialization(stream_service):
    with patch("api.services.stream_service.StreamChat") as mock_chat, patch(
        "api.services.stream_service.Stream"
    ) as mock_feed:
...
def test_upsert_user(stream_service):
    ...
    result = stream_service.upsert_user(
        "user123", {"name": "Test User", "role": "trader"}
    )

    assert result is True
    stream_service.chat_client.upsert_user.assert_called_once_with(
        {"id": "user123", "name": "Test User", "role": "trader"}
    )
