...
from api.services.asset_manager import create_asset_manager, Position, ClosedTrade, AccountSummary
...
class IntegratedTradingSystem:
    ...
    async def initialize(self):
        ...
        try:
            ...
            self.services['asset_manager'] = await create_asset_manager(use_google_sheets=True)
            
            # Initialize EA server
            logger.info("Starting EA communication server...")
            self.services['ea_server'] = await create_ea_server(
                host=settings.EA_SERVER_HOST, 
                port=settings.EA_SERVER_PORT
            )
        ...
        try:
            ...
            for instrument in instruments:
                ...
                if candles:
                    ...
                    logger.info(f"Training results for {instrument}: {training_results}")
    ...
    def _setup_event_handlers(self):
        ...
        self.services['ea_server'].subscribe_to_account_status(self._handle_account_status)
    ...
    async def _handle_account_status(self, account_status, connection):
        ...
        try:
            logger.info(f"Account status update: Balance={account_status.balance}, "
    ...
    async def _generate_trading_signal(self, instrument: str, market_data):
        ...
        try:
            ...
            news_articles = await self.services['news'].get_recent_news(
                keywords=[instrument.replace('_', '/'), 'forex'],
                hours_back=24
            )
            ...
            sentiment = await self.services['gemini'].analyze_market_sentiment(
                news_articles=[article.get('content', '') for article in news_articles[:10]],
                social_media_posts=[post.get('content', '') for post in social_posts[:20]],
                instrument=instrument
            )
            ...
            if not validation.is_valid:
                logger.warning(f"Signal validation failed for {instrument}: {validation.reasoning}")
                return
            
            # Adjust confidence based on Gemini validation
            adjusted_confidence = prediction.confidence * validation.confidence_adjustment
            
            if adjusted_confidence < 0.6:  # Minimum confidence threshold
                logger.info(f"Adjusted confidence too low for {instrument}: {adjusted_confidence}")
                return
            ...
            base_volume = risk_params.max_risk_per_trade * account_info.get('equity', 10000) / 10000
            ...
            if success:
                ...
                logger.info(f"Trading signal sent for {instrument}: {prediction.signal} "
    ...
    async def _periodic_maintenance(self):
        ...
        try:
            ...
            if current_time.minute % 5 == 0:
                ...
                logger.info(f"System Status - Signals Generated: {self.signals_generated}, "
